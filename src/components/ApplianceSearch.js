...
const ApplianceSearch = ({ onSearch }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.post('https://6hxx73ulo2.execute-api.us-east-1.amazonaws.com/prod/search', { serial_number:serialNumber, brand, model, date_bought:dateBought,status ,

      });

      setSearchResults(response.data["body-json"]["data"]);
